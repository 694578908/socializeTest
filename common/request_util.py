...
class RequestUtil:
    ...
    def send_requests(self, title, method, url, headers, data, **kwargs):
        ...
        if method == 'get':
            rep = RequestUtil.session.request(method=method, url=url, params=data, headers=headers, timeout=timeout_value,
                                              **kwargs)
        else:
            data = json.dumps(data)
            rep = RequestUtil.session.request(method=method, data=data, url=url, headers=headers, timeout=timeout_value, **kwargs)

        end_time = time.time()
        elapsed_time = end_time - start_time
        elapsed_time_rounded = round(elapsed_time, 2)

        response_timeout = {'接口预期请求时间': timeout_value, '接口实际请求时间': elapsed_time_rounded}
        ...
        log_util.log_info(
            '{}>>>>接口状态码:{},预期请求时间:{}秒,实际请求时间:{}秒'.format(log_status, rep.status_code, timeout_value, elapsed_time_rounded))
